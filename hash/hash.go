...
/*思路*/
/*
1.找出哪种礼包打折力度最大，也就是单价/原总价最小，按打折力度排序
2.在不超单类物品数量限制前提下，尽可能多的买这类礼包
...
func ShoppingOffers(price []int, special [][]int, needs []int) int {
	...
	for _, gift := range special {
		isValid := true
		tempPrice := 0
		for ind := 0; ind < numGoods; ind++ {
			if needs[ind] < gift[ind] {
				isValid = false
			}
			tempPrice = gift[ind]*price[ind] + tempPrice
		}
		if isValid && tempPrice > gift[numGoods] {
			newSpecial = append(newSpecial, gift)
		}
	}

	//如果newSpecial为空，所有东西都要单买
	if len(newSpecial) == 0 {
		tempSum := 0
		for ind, cnt := range needs {
			tempSum = tempSum + cnt*price[ind]
		}
		return tempSum
	}
	...
	//一个个礼包尝试
	curMin := 0
	for _, gift := range newSpecial {
		leftNeeds := make([]int, numGoods)
		for ind := 0; ind < numGoods; ind++ {
			leftNeeds[ind] = needs[ind] - gift[ind]
		}
		tempRes := ShoppingOffers(price, newSpecial, leftNeeds)
		tempRes = tempRes + gift[numGoods]
		if tempRes < curMin || curMin == 0 {
			curMin = tempRes
		}
	}
	return curMin
